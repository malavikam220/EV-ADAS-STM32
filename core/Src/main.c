...
/*----------------UART RX byte---------------------*/
static uint8_t rx_byte;	/*initialize the receiving byte*/


/*----------------Counter---------------------*/
static uint8_t ev_div=0;	/*print every 10 EV ticks*/
//static uint32_t loop_count=0;	/*main loop iteration count*/

char msg[100];

void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	  if(htim->Instance==TIM1) flag_ev=1;     /* 10 ms EV tick, read values from pot. when flag is high */
	  if(htim->Instance==TIM3) sensor_flag=1; /*set the flag every 100ms, read data from usr every 100ms */
...
int main(void)
{
	  ...
	  HAL_TIM_Base_Start_IT(&htim2); 	   /* counter, counting for every 1us */
	  HAL_UART_Receive_IT(&huart1,&rx_byte,1);								/* Start interrupt on receiving the data*/


	  EV_Init(&ev);		 	//ev initialization
	  HCSR04_Init();	 	//ultrasonic sensor initialization
	  ADAS_Init(&adas);		// initialize l f r as 400 cm
	  Fault_Init(&flt);		// initialize flag, active=0
	  Shell_Init(&huart1,&ev,&adas,&flt);	//initialize the shell
	  ...
	  while (1)
	  {
	      ...
	      if (flag_ev) {
	          ...
	          if (++ev_div >= 10) {
	              ...
	              Print_Status(); //available in uart.c

	          }
	      }

	      if (sensor_flag==1) 		// for every 100ms
	      {
	    	  sensor_flag=0;

	    	  // read ultrasonic sensor (hsrc) data and print it
	    	  HCSR04_ReadAll();

	    	  // update TTC, alarms,BSD
	    	  ADAS_Update(&adas,&ev);

	    	  /* validate motor temp, SOC and collision and update the fault flags*/

	    	  Fault_Check(&flt,&ev,&adas);

	    	  // convert them to integer (decimal) and print them (code deleted)

	      }
	    	  Shell_Process();	//waiting to receive the data and process the command.
