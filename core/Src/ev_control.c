...
void EV_Update(EV_HandleTypeDef *ev, float dt)
{
    ...
    float pedal = ev->accel_pedal / 100.0f;				  //ev->motor_torque = (ev->accel_pedal / 100.0f) * EV_MAX_TORQUE_NM * mode_scale;

    pedal = sqrtf(pedal);      // quicker response

     ev->motor_torque = pedal * EV_MAX_TORQUE_NM * mode_scale;
